...
@export var action_prefix: String = "player2"
...
@onready var football_opponent_anim: AnimatedSprite2D = $AnimatedSprite2D
...
func _ready() -> void:
	...
	football_opponent_anim.play("idle")
	if ball == null:
...
func handle_ball(delta: float, input: Vector2) -> void:
	...
	if has_ball():
		...
		cancel_charge()
		if Input.is_action_just_pressed("player2_tackle") and _tackle_timer <= 0.0 and _stun_timer <= 0.0:
...
func update_animation(input: Vector2) -> void:
	if input != Vector2.ZERO and _stun_timer <= 0.0:
		if football_opponent_anim.animation != "walk":
			football_opponent_anim.play("walk")
	else:
		if football_opponent_anim.animation != "idle":
			football_opponent_anim.play("idle")
